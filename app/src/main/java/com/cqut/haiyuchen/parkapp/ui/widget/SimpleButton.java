...
import com.cqut.haiyuchen.parkapp.ui.untils.MeasureUtils;

/**
 * Created by chy on 2017/4/11.
...
public class SimpleButton extends View {
  ...
  private void _init(Context context, AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      try {
        ...
        if (mTagMode == MODE_CHECK
            || mTagMode == MODE_ICON_CHECK_INVISIBLE
            || mTagMode == MODE_ICON_CHECK_CHANGE) {
        ...
        mBorderColor =
            a.getColor(R.styleable.SimpleButton_sb_border_color, Color.parseColor("#ff333333"));
        mTextColor =
            a.getColor(R.styleable.SimpleButton_sb_text_color, Color.parseColor("#ff666666"));
        ...
        mHorizontalPadding = (int) a.getDimension(R.styleable.SimpleButton_sb_horizontal_padding,
            mHorizontalPadding);
        mVerticalPadding =
            (int) a.getDimension(R.styleable.SimpleButton_sb_vertical_padding, mVerticalPadding);
    ...
    if (mTagMode == MODE_ICON_CHECK_CHANGE && mDecorateIconChange == null) {
      throw new RuntimeException(
          "You must set the drawable by 'tag_icon_change' property in MODE_ICON_CHECK_CHANGE mode");
    }
  ...
  private int _adjustText(int maxWidth) {
    if (mPaint.getTextSize() != mTextSize) {
      ...
      mBaseLineDistance =
          (int) Math.ceil((fontMetrics.descent - fontMetrics.ascent) / 2 - fontMetrics.descent);
    }
    // 计算文字宽度
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int width =
        (MeasureSpec.getMode(widthMeasureSpec) == MeasureSpec.EXACTLY) ? MeasureSpec.getSize(
            widthMeasureSpec) : allPadding + fontLen;
    int height =
        (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) ? MeasureSpec.getSize(
            heightMeasureSpec) : mVerticalPadding * 2 + mFontH;
